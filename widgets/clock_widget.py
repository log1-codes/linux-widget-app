...
class AnalogClockWidget(BaseWidget):
    ...
    def _build_clock(self):
        ...
        provider.load_from_data(CLOCK_CSS.encode())
    ...
    def _on_draw_clock(self, widget, cr):
        ...
        # Outer ring
        cr.set_source_rgba(0.10, 0.10, 0.18, 0.70)
        cr.arc(cx, cy, radius + 6, 0, 2 * math.pi)
        cr.fill()

        # Face
        cr.set_source_rgba(0.15, 0.15, 0.25, 0.55)
        ...
        for i in range(60):
            ...
            if i % 5 == 0:
                inner_r, outer_r = radius - 16, radius - 4
                cr.set_source_rgba(0.85, 0.88, 0.96, 0.90)
                cr.set_line_width(2.5)
            else:
                inner_r, outer_r = radius - 10, radius - 4
                cr.set_source_rgba(0.60, 0.65, 0.80, 0.35)
        ...
        # Hour hand – macOS Blue
        hour_angle = (hours + minutes / 60.0) * 30 * math.pi / 180 - math.pi / 2
        cr.set_source_rgba(0.39, 0.71, 0.96, 0.95)
        cr.set_line_width(4)
        cr.set_line_cap(2)
        cr.move_to(cx, cy)
        cr.line_to(cx + radius * 0.50 * math.cos(hour_angle),
                   cy + radius * 0.50 * math.sin(hour_angle))
        cr.stroke()

        # Minute hand
        min_angle = (minutes + seconds / 60.0) * 6 * math.pi / 180 - math.pi / 2
        cr.set_source_rgba(0.76, 0.83, 0.96, 0.90)
        cr.set_line_width(2.5)
        cr.move_to(cx, cy)
        cr.line_to(cx + radius * 0.70 * math.cos(min_angle),
                   cy + radius * 0.70 * math.sin(min_angle))
        cr.stroke()

        # Second hand – macOS Orange/Red
        sec_angle = seconds * 6 * math.pi / 180 - math.pi / 2
        cr.set_source_rgba(1.0, 0.27, 0.23, 0.88)
        cr.set_line_width(1.5)
        cr.move_to(cx, cy)
        cr.line_to(cx + radius * 0.75 * math.cos(sec_angle),
                   cy + radius * 0.75 * math.sin(sec_angle))
        cr.stroke()

        # Centre dot
        cr.set_source_rgba(1.0, 0.27, 0.23, 0.95)
        cr.arc(cx, cy, 4, 0, 2 * math.pi)
        cr.fill()
        cr.set_source_rgba(0.85, 0.88, 0.96, 0.6)
